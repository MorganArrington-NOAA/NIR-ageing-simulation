...
# Run PCA
m <- pca(all_dat[,7:505], ncomp = 10, scale= F, center = F)

# Might need to do some outlier removal before getting better average
plotResiduals(m, ncomp = 5, show.labels = T) #239, 617, 378

# filter out outliers
all_dat_or <- filter(all_dat[-c(239,378,617),])
...
# Remove
all_dat_or <- filter(all_dat_or[-364,])
...
# Remove
all_dat_or <- filter(all_dat_or[-808,])
...
all_dat_L<-all_dat_or%>%tidyr::pivot_longer(.,cols=c(6:505),names_to="wavenumber",values_to="absorbance", names_prefix = "x") 
...
scores <- scores%>%
  mutate(PC1_stand = ((PC1-5)^2)/m2$eigenvals[1], PC2_stand = ((PC2+5)^2)/m2$eigenvals[2], PC3_stand = ((PC3+5)^2)/m2$eigenvals[3])
...
summ <- scores %>% # calculate mean and get feel for error
  group_by(Age) %>%
  summarize(mean_stand = across(c(PC1_stand:PC3_stand), ~mean(.)), se_stand = across(c(PC1_stand:PC3_stand), ~sd(.)/sqrt(n())))
...
# I think PC1-PC3 will be plenty to capture variability by age

# Now that I have standardized scores, need to draw random error around average PC by age for each PC1-3. I already have averages calculated in summ tibble. So, lets make a list "samp", make a loop which will draw error around mean = 0, sd = 0.01

#set up dataframe with 10 records per age (1-10 years)
summ_rep <- summ%>% 
  slice(rep(1:n(), each = 10)) #however many datapoints we want per age
...
for (k in 1:Iter){ # make Iter objects in a list 
  samp[[k]] <- summ_rep %>% 
    rowwise()%>%
    dplyr::transmute(PC1_new_score = rnorm(1,mean = mean_stand$PC1_stand, sd = 0.1), PC2_new_score = rnorm(1,mean = mean_stand$PC2_stand, sd = 0.1), PC3_new_score = rnorm(1,mean = mean_stand$PC3_stand, sd = 0.1), Age)
}
...
standscores_to_raw <- function(df) {
 df%>%
    transmute(PC1_sim = sqrt(PC1_new_score*m2$eigenvals[1])*-1+5, PC2_sim = sqrt(PC2_new_score*m2$eigenvals[2])-5, PC3_sim = sqrt(PC3_new_score*m2$eigenvals[3])-5, Age)
}
...
# Pivot for plotting
test_L<-test_df%>%tidyr::pivot_longer(.,cols=c(3:501),names_to="wavenumber",values_to="absorbance", names_prefix = "x") 
...
m_sim <- pca(test_df[,3:501], ncomp = 10, scale= F, center = F)
