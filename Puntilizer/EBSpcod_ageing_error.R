...
#Load data
MetaData <- read.csv("C:/Users/marri/OneDrive/Documents/AFSC A&G Contract/Simulation Project/Data/MetaData_2010-2019_n14579.csv")
...
Nreaders <-dim(AgeReads_71)[2]
# make table formatted as required by ageing error software
Reads2 = data.frame(count=1, AgeReads_71[1,])
# loop over rows of original data
for(RowI in 2:nrow(AgeReads_71)){
  DupRow <- NA
  ...
  for(PreviousRowJ in 1:nrow(Reads2)){
    ...
    if(all(AgeReads_71[RowI,1:Nreaders]==
  ...
  if(is.na(DupRow)){
    ...
    Reads2 <- rbind(Reads2, data.frame(count=1, AgeReads_71[RowI,]))
  }
...
model.name <-c("B0_S1_S0","B0_S2_S0","B0_S3_S0")
